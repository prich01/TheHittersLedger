...
class _CageRoutinesScreenState extends State<CageRoutinesScreen> {
  final List<String> _savedRoutines = [];
  final TextEditingController _routineController = TextEditingController();

  // THE NEW SHARE FUNCTION
  void _shareRoutine(String routine) {
    final String message = "⚾ My Hitter's Ledger Routine:\n\n$routine\n\nGenerated by The Hitter's Ledger";
    
    // Web-safe share call for Zapp/Browsers
    Share.share(
      message,
      subject: 'My Baseball Routine',
      sharePositionOrigin: const Rect.fromLTWH(0, 0, 10, 10),
    );
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
            ...
            child: Column(
              children: [
                TextField(
                  ...
                  decoration: const InputDecoration(hintText: "ENTER NEW ROUTINE..."),
                ),
                const SizedBox(height: 16),
                SizedBox(
                  width: double.infinity,
                  ...
                  child: ElevatedButton(
                    ...
                    onPressed: () {
                      if (_routineController.text.isNotEmpty) {
                        setState(() => _savedRoutines.add(_routineController.text));
          ...
          const Divider(color: Colors.white10),
          Expanded(
            child: _savedRoutines.isEmpty
                ? const Center(child: Text("NO ROUTINES SAVED", style: TextStyle(color: Colors.white24, letterSpacing: 1.5)))
                : ListView.builder(
                    itemCount: _savedRoutines.length,
                    itemBuilder: (context, index) {
                      final routine = _savedRoutines[index];
                      return Container(
                        margin: const EdgeInsets.symmetric(horizontal: 24, vertical: 8),
                        decoration: BoxDecoration(color: const Color(0xFF1A1D21), borderRadius: BorderRadius.circular(12)),
                        child: ListTile(
                          leading: const Icon(Icons.check_circle_outline, color: Color(0xFFD4AF37)),
                          title: Text(routine, style: const TextStyle(fontWeight: FontWeight.bold)),
                          trailing: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              // NEW SHARE BUTTON
                              IconButton(
                                icon: const Icon(Icons.share, color: Colors.blueAccent, size: 20),
                                onPressed: () => _shareRoutine(routine),
                              ),
                              IconButton(
                                icon: const Icon(Icons.delete_outline, color: Colors.redAccent, size: 20),
                                onPressed: () => setState(() => _savedRoutines.removeAt(index)),
                              ),
                            ],
                          ),
                        ),

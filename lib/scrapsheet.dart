...
Future<void> main() async {
  // This is the anchor that holds everything together
  ...
  try {
    ...
    // If the internet is totally out or Firebase is down
    print("❌ CLOUD STATUS: Connection failed. Error: $e");
  }

  // 2. Launch the app (It will now have the connection ready)
...
class _HitterLogScreenState extends State<HitterLogScreen> {
        ...
        appBar: AppBar(
          ...
          actions: [
            Padding(
              ...
              ), // <--- This closes ActionChip
            ), // <--- This closes Padding
          ], // <--- This closes the actions list
              ),
            ),
          ],
          // ---------------------------
  ...
  MaterialPageRoute(builder: (context) => EntryForm(
    pData: _pData, 
    activeSeason: _activeSeason, // <-- Add this line here!
  ))
);

  if (result != null && result is AtBatLog) {
    // 1. UPDATE UI IMMEDIATELY
    // We do this first so the hitter sees their data right away
    // setState(() => _allLogs.insert(0, result));
    _saveLogs(); // Save local backup

    // 2. TRIGGER CLOUD SAVE (No 'await')
    // Removing 'await' allows the app to stay snappy even if the internet is slow
    saveAtBat({
    ...
    }).catchError((e) => print("⚾ Cloud Save Error: $e"));
  }
},
child: const Icon(Icons.add, color: Colors.black),
  ...
  } // Closes the build method (This was missing!)
} // Closes the _HitterLogScreenState class (This was missing!)
      ...
      Expanded(
        child: StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            // 3. Debug Print
            Future.microtask(() {
              print("⚾ DEBUG: Cloud found ${cloudDocs.length} documents.");
            });

            // 4. Map to Objects
            final List<AtBatLog> seasonalLogs = cloudDocs.map((doc) {
              return AtBatLog.fromJson(doc.data() as Map<String, dynamic>);
            }).toList();

            // 5. Apply Season and Search Filters
            final filteredLogs = seasonalLogs.where((log) {
              final matchesSeason = log.season == _activeSeason;
              final matchesSearch = log.pitcher.toLowerCase().contains(query);
              return matchesSeason && matchesSearch;
            }).toList();

            // 6. UI Logic (The fallback message)
            if (filteredLogs.isEmpty) {
              return Center(
                child: Text(
                  "NO RECORDS FOUND FOR $_activeSeason",
                  style: const TextStyle(color: Colors.white24, letterSpacing: 1.5, fontSize: 12),
                ),
              );
            }

            // 7. SUCCESS: Return the list of cards
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        const SizedBox(height: 32),
        SizedBox(width: double.infinity, height: 55, child: ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFD4AF37), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), onPressed: () => Navigator.pop(context, AtBatLog(pitcher: _pitcher.text, team: _team.text, hand: _hand, velocity: _velo.text, result: _res, pitches: List.from(_sequence), notes: _notes.text, date: _date.text, gameLabel: "", abNumber: _selectedAB, isQAB: _qab, swingThought: _swingThought.text, season: widget.activeSeason)), child: const Text("SAVE TO LEDGER", style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900, letterSpacing: 1.5)))),
        const SizedBox(height: 40),
      ])),
    );
  }
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<User?>(
      ...
      builder: (context, snapshot) {
        ...
        return const AuthScreen(); // Added 'const' here to match your other error
      },
    );
  }
}
...
Future<void> saveAtBat(Map<String, dynamic> atBatData) async {
  final user = FirebaseAuth.instance.currentUser;
  
  // 1. Safety Check: If no user is logged in, we can't save to their account
  if (user == null) {
    print("⚾ No user logged in. Save cancelled.");
    return;
  }
  ...
  try {
    ...
    // 2. Use a 'WriteBatch' so both the log and the counter happen together
    WriteBatch batch = FirebaseFirestore.instance.batch();

    // 3. Prepare the At-Bat record
    DocumentReference newAtBat = userDoc.collection('logs').doc();
    batch.set(newAtBat, {
      ...atBatData,
      'timestamp': FieldValue.serverTimestamp(),
    });

    // 4. Prepare the Paywall Increment AND Identify the User
    // We add 'email' and 'displayName' here so you can identify them in the console
    batch.set(userDoc, {
      'email': user.email,
      'displayName': user.displayName ?? "Unknown Hitter",
    ...
    // 5. Fire!
    await batch.commit();
    print("⚾ Firestore: Log saved and paywall counter increased!");
